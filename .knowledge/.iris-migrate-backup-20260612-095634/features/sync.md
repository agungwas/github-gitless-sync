...
- **CRITICAL BUG (Background Sync Race Condition)**: There is a race condition in `syncImpl()` if the user is typing while a background sync runs. `syncImpl` first reads the file's content into memory (`newTreeFiles`) to upload to GitHub. Later, in `commitSync`, it calls `calculateSHA(filePath)` to update the local metadata's `sha`. If the user continues typing between the read and `calculateSHA`, the metadata gets updated with the *newer* SHA, while GitHub receives the *older* content. On the next sync, `actualLocalSHA === localFile.sha` (plugin thinks no local changes happened), but `remoteFile.sha !== actualLocalSHA` (because GitHub has the older SHA). The plugin incorrectly assumes the remote was updated and triggers a `download`, silently overwriting the user's newest changes with the older uploaded version.
